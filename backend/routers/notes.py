...
def read_notes(
    user_id: int, folder: Optional[str] = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    return crud.get_user_notes(db, user_id=user_id, folder=folder, skip=skip, limit=limit)

@router.put("/{note_id}", response_model=schemas.Note)
def update_note(
    user_id: int, note_id: int, note_update: schemas.NoteUpdate, db: Session = Depends(get_db)
):
