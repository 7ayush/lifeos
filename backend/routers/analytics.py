...
def _compute_user_entry(db: Session, user: models.User) -> dict:
    """Compute a single user's leaderboard entry."""
...
def get_leaderboard(
    ...
    """
    users = db.query(models.User).filter(models.User.profile_visibility == "public").all()
    leaderboard = []

    for user in users:
        leaderboard.append(_compute_user_entry(db, user))

    # Always include the authenticated user's own entry (Req 7.4)
    if not any(entry["user_id"] == current_user.id for entry in leaderboard):
        leaderboard.append(_compute_user_entry(db, current_user))
