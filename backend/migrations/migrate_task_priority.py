...
This adds a 'priority' field with valid values: "High", "Medium", "Low", "None".
Existing rows are set to "None" by default.

Usage:
    python -m backend.migrate_task_priority
    python -m backend.migrate_task_priority downgrade
...
def upgrade():
    ...
    if not column_exists(cursor, "tasks", "priority"):
        cursor.execute("ALTER TABLE tasks ADD COLUMN priority TEXT DEFAULT 'None'")
        cursor.execute("UPDATE tasks SET priority = 'None' WHERE priority IS NULL")
        print("  ✓ Added column: priority")
    else:
        print("  – Column already exists: priority")
...
def downgrade():
    ...
    if column_exists(cursor, "tasks", "priority"):
        cursor.execute("PRAGMA table_info(tasks)")
        columns_info = cursor.fetchall()
        columns = [col for col in columns_info if col[1] != "priority"]
        ...
        print("  ✓ Dropped column: priority")
    else:
        print("  – Column does not exist: priority")
