...
import { Plus, Trash2, Calendar, GripVertical, ChevronLeft, ChevronRight, CalendarDays, LayoutTemplate, Archive, Check } from 'lucide-react';
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col animate-in fade-in duration-500 relative z-10">
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className={`flex-1 grid grid-cols-1 ${
        ...
        } gap-6 min-h-0 overflow-y-auto pb-6 custom-scrollbar`}>
          {visibleColumns.map((colConfig) => {
            ...
            return (
              <div key={colConfig} className={`flex flex-col h-full ${cStyle.bg} backdrop-blur-3xl border ${cStyle.border} rounded-[2rem] p-5 ${cStyle.glow} relative overflow-hidden transition-all duration-500`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={() => setIsModalOpen(false)}>
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 shadow-2xl border border-white/10" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-2xl font-bold text-white mb-6 font-['Outfit']">Create New Task</h2>
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                >
                  {loading ? <span className="w-4 h-4 rounded-full border-2 border-black/30 border-t-black animate-spin" /> : "Create Task"}
                </button>
              </div>
            </form>
          </div>
