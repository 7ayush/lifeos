    ...
    <div ref={menuRef} className="fixed bottom-8 right-8 z-50">
          ...
          {CAPTURE_OPTIONS.map((opt, i) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl border border-white/10 backdrop-blur-xl bg-[#0a0a0a]/90 hover:bg-white/5 transition-all duration-300 group whitespace-nowrap ${opt.glow}`}
              ...
              >
                ...
                <span className="text-sm font-bold text-white font-['Outfit'] tracking-tight">{opt.label}</span>
              </button>
            );
          })}
        </div>
      ...
      {activeCapture && (
        <div className="absolute bottom-20 right-0 w-80 animate-in fade-in zoom-in-95 duration-200">
          <div className="glass-panel rounded-2xl border border-white/10 p-5 shadow-2xl backdrop-blur-xl bg-[#0a0a0a]/95">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-neutral-600 focus:outline-none focus:border-indigo-500/50 transition-colors text-sm"
            />
            <div className="flex items-center justify-between mt-4">
              <p className="text-[10px] text-neutral-600 font-medium">Press <kbd className="px-1.5 py-0.5 bg-white/5 rounded text-neutral-400 border border-white/10 text-[9px]">Enter</kbd> to save</p>
              <button
      ...
      <button
        ...
        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-2xl active:scale-90 ${
          isOpen
            ? 'bg-white/10 border border-white/20 rotate-45 shadow-[0_0_30px_rgba(255,255,255,0.1)]'
